...
def send_angles_to_arduino(ser, roll, pitch1, pitch2, pitch3):
    if ser:
...
def animate(frame):
    ...
    current_z = 4.3 # Keep z constant at height

    roll, pitch1, pitch2, pitch3 = inverse_kinematics([current_x, current_y, current_z])

    # Send angles to Arduino
    send_angles_to_arduino(ser, np.degrees(roll), np.degrees(pitch1), np.degrees(pitch2), np.degrees(pitch3))
...
ser = initialize_serial('COM6')  # Replace 'COM6' with your Arduino's port
